...
test('Section_04_RS_UI_Tests - Change Focus From Child To Parent Window', async ({browser} )=> {
    ...
    console.log("Filling 'Gaurav khope' in username textbox present on parent window");
    await page.locator('#username').fill("Gaurav khope");

    //await page.pause();

    console.log("await page.locator('#username').textContent(): " + await page.locator('#username').textContent());
    
    // console.log("Assertion for username textbox - It should contain text: 'Gaurav khope'");
     await expect(page.locator('#username')).toContainText('Gaurav khope');
     //await expect(page.locator('#username')).toHaveText('Gaurav khope');
     //await expect(page.locator('#username')).toHaveValue('Gaurav khope');
     //await expect(page.locator('#username')).toHaveAttribute('value', 'Gaurav khope');
