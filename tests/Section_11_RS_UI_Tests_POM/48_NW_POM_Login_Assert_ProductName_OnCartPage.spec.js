...
test('Section_11_RS_UI_Tests - RahulShettyAcademy Client App Login - POM_Login_Assert_ProductName_OnCartPage', async ({page} )=> {
    ...
    const product_ToAdd='ZARA COAT 3';
    ...
    // delete from cart and assert for empty cart
    await cartPage.removeProduct_FromCart(product_ToAdd);
console.log(`${product_ToAdd} removed from cart, now asserting for empty cart message...`);
    await expect(page.getByText("No Products in Your Cart !")).toBeVisible();
    console.log("Now cart is empty, assertion for empty cart message is successful!");
