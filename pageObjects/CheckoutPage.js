class CheckoutPage{
    ...
    async searchCountryAndSelect(inputToTextBox,country_ToSearch){
        ...
        await this.drpdwn_selectCountry.type(inputToTextBox, { delay:1000 } );

        console.log("Waiting for Countries to load after search for '"+inputToTextBox+"'");
         await this.options_country.waitFor();

        const countriesCount =  await this.options_country.locator('button').count();

        for(let index=0;index<countriesCount;index++){
            const countryName = await this.options_country.locator('button').locator('span').nth(index).textContent();
